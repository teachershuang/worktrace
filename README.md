...
- 桌面版和托盘版启动后自动进入后台记录循环，并保留用户上次的暂停状态。
- 在指定工作时间段内按周期截图并记录。
- 获取当前应用名、窗口标题和系统空闲时间。
...
3. 双击 `WorkTrace.exe`。首次启动会在 EXE 同目录生成 `config.yaml`，后台记录循环会自动启动。
4. 打开左侧“接口配置”，填写 OCR 地址、LLM 地址、API Key 和模型名称。
5. 分别执行“测试 OCR”和“测试 LLM”。
6. 配置生效后等待第一个记录周期，或点击“立即记录一次”验证完整链路；若此前暂停过，再点击“开始 / 恢复”。
...
桌面版启动后会自动进入后台记录循环，并保留上次暂停状态。首次配置完成后，点击桌宠打开快捷面板，选择“立即记录一次”可立即验证链路。一次完整记录流程为：
...
```powershell
7z a -t7z -mx=9 dist\WorkTrace-v0.4.1-windows-x64.7z .\dist\WorkTrace\*
```
